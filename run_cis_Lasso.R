...
option_list <- list(
  ...
  make_option(c("-n", "--nfold"), type="integer", default=5,
              ...
              metavar="character"),
  make_option(c("--inner_nfold"), type="integer", default=5,
  ...
  make_option(c("-d", "--mode"), type="character", default='cv_performance',
              ...
              metavar="character"),
  make_option(c("--gene_annot"), type="character", default=NULL,
              help="read in gene annotation file in RDS.",
              metavar="character"),
  make_option(c("--snp_annot"), type="character", default=NULL,
              help="read in snp annotation file in RDS.",
              metavar="character")
)
...
# helper functions
source('/gpfs/data/im-lab/nas40t2/festus/metabolomics/guardian/MetaboXcan/scripts/gw_lasso/gw_lasso_helper.R')
library(dplyr)
...
#load annotation files to  only include cis snps
gene_annot = readRDS(opt$gene_annot)
rownames(gene_annot) <- gene_annot$gene_id
snp_annot = readRDS(opt$snp_annot)
...
for(pheno in colnames(df_phenotype)[c(-1, -2)]) {
  ...
  for(k in 1 : opt$nfold) {
    ...
    inner_fit = snpnet::snpnet(
      ...
      phenotype = pheno,
      split.col = "split",
      configs =  list(nCores = 4, niter = 50, use.glmnetPlus = TRUE, num.snps.batch = 200, plink2.path = '/gpfs/data/im-lab/nas40t2/yanyul/softwares/plink2', zstdcat.path = "/gpfs/data/im-lab/nas40t2/festus/my_envs/snpnet/bin/zstdcat", excludeSNP =minorsnps),
      alpha = opt$alpha,
      mem = opt$mem,
    )
