...
get_filtered_snp_annot <- function(snp_annot_file_name) {
  snp_annot <- readRDS(snp_annot_file_name) %>%
...
get_maf_filtered_genotype <- function(genotype_file_name,  maf, samples) {
  ...
  names(gt_df) <- substring(names(gt_df), 2)
...
get_gene_annotation <- function(gene_annot_file_name, chrom, gene_types=c(' protein_coding', ' pseudogene', ' lincRNA')){
  gene_df <- readRDS(gene_annot_file_name) %>%
...
adjust_for_covariates <- function(expression_vec, cov_df) {
  ...
  fit <- lm(combined_df[,1] ~ combined_df$sex + combined_df$batchnumber + combined_df$PF1 + combined_df$PF2 + combined_df$PF3 + combined_df$PF4 + combined_df$PF5 + combined_df$PF6 + combined_df$PF7)
...
main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file,
                 covariates_file, chrom, prefix, maf=0.01, n_folds=10, n_train_test_folds=2,
  ...
  tiss_chr_summ_f <- prefix %&% '_chr' %&% chrom %&% '_summary.txt'
